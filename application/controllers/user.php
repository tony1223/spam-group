...
class User extends MY_Controller {
	...
	public function js_report_uid(){
		...
		$check_reporter = $this->UserModel->check_reported($uid,$reporter);
		echo json_encode(Array("user"=>$user , "reported" =>
			$check_reporter ) );
	}
...
//		die('{"IsSuccess":true,"Data":119}');
	...
	public function js_report_user(){
		...
		$userid = $this->UserModel->insert_report(
			Array(
				"Name" => $name ,
				"UID" => $uid,
				"ReporterFBUID" => $report_uid,
		...
		$this->UserModel->mark_as($uid, $report_uid, false);

		if($userid == -1){
			echo json_encode(Array("IsSuccess" => false, "ErrorCode" => 2 , "ErrorMessage" => "回報社團時發生意外錯誤，可能已回報過" ));
			return false;
		}
		echo json_encode(Array("IsSuccess" => true, "Data" => $userid));
	}
